...
export default class MyDatePicker extends Component {
    ...
    getDayDetails =args=> {
        ...
        */
        let date = args.index - args.firstDay; //-4
        let day = args.index%7;//0
        let prevMonth = args.month-1;//2(mar)
        let prevYear = args.year;//2021
        if(prevMonth < 0) {
            prevMonth = 11;
            prevYear--;
        }
        let prevMonthNumberOfDays = this.getNumberOfDays(prevYear, prevMonth);//31
        let _date = (date < 0 ? prevMonthNumberOfDays+date : date % args.numberOfDays) + 1;
        let month;//date < 0  ? -1 : date >= args.numberOfDays ? 1 : 0;
        // if(this.props.prevDate && (new Date(prevYear,args.month,date+1).getTime() > todayTimestamp)){
        //     month=0;
        // }
        if(date<0 || ((!this.props.prevDate) && (new Date(prevYear,args.month,date+1).getTime() > todayTimestamp))){
            month=-1;
        }
        else if(date>=args.numberOfDays){
            month=1;
        }
        else if(this.props.prevDate && date>=0 && (this.props.prevDate > (new Date(prevYear,args.month,date)).getTime())){
            month=-1;
        }
        else{
            month=0;
    ...
    getMonthDetails =(year, month)=> {
        let firstDay = (new Date(year, month)).getDay();//4(thu) if year = 2021 and month=3(apr)
        let numberOfDays = this.getNumberOfDays(year, month);//30
        let monthArray = [];
        let rows = 6;
        let currentDay = null;
        let index = 0; 
    ...
    updateDateFromInput =()=> {
        ...
        let dateValue = this.state.currentDate
        ...
        return (
            <div className='MyDatePicker'>
                <div className='mdp-input'  onClick={()=> this.showDatePicker(true)}>
                    <input type='text' onChange={this.updateDateFromInput} value={this.state.currentDate}/>
                </div>
